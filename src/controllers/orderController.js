...
const getAllOrders = catchAsync(async (req, res, next) => {
  ...
  // Fetch paginated orders with associated user data
  const {
    data: ordersData,
    error,
    count,
  ...
  const ordersWithItems = await Promise.all(
    ordersData.map(async (order) => {
      const { data: orderItemsData, error: orderItemsError } = await supabase
        ...
        .eq("order_id", order.order_id); // Match by order_id
      ...
      return {
        ...
        status: order.status, // Add the fetched order items here
      };
    })
  );
  ...
  const ordersWithItems = await Promise.all(
    ordersData.map(async (order) => {
      ...
      return {
        ...
        created_at: order.created_at,
        total_price: order.total_price,
        order_items: orderItems, // Add the fetched order items here
        status: order.status,
      };
    })
  );

  // Send the refined order data with items as response
...
const addOrder = catchAsync(async (req, res, next) => {
  ...
  // Fetch the cart items by cart_id
  ...
  // Insert each cart item as an order item in the 'order_items' table
