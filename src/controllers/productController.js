...
// });
const getAllProducts = catchAsync(async (req, res, next) => {
  // Extract category, minPrice, and maxPrice from query parameters
  ...
  const page = req.query.page || 1; // default to 'all' if no category is specified
  console.log(page);
  const offset = (page - 1) * limit;
  const minPrice = parseFloat(req.query.minPrice) || 0; // default minimum price is 0
  const maxPrice = parseFloat(req.query.maxPrice) || Infinity; // default maximum price is Infinity
...
const getProductsBySearch = catchAsync(async (req, res, next) => {
  const search = req.query.search || ""; // default to 'all' if no category is specified
