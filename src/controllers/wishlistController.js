...
const removeWishlistItem = catchAsync(async (req, res, next) => {
  ...
  // Ensure both product_id and user_id are provided
  if (!product_id || !user_id) {
    return throwError(res, "There is missing field");
  }

  // Step 1: Get the wishlist_id for the given user_id
  const { data: wishlistData, error: wishlistError } = await supabase
    .from("wishlists")
    .select("wishlist_id") // Assuming the wishlist ID column is named 'id'
    .eq("user_id", user_id)
    .single(); // Assuming one wishlist per user

  // Handle error or case where no wishlist is found
  if (wishlistError || !wishlistData) {
    return throwError(res, "Wishlist not found");
  }

  const wishlist_id = wishlistData.wishlist_id; // Get the wishlist_id

  // Step 2: Delete the wishlist item using wishlist_id and product_id
  const { data: removeData, error: removeError } = await supabase
    .from("wishlist_items")
    .delete()
    .eq("wishlist_id", wishlist_id) // Use wishlist_id to filter
    .eq("product_id", product_id); // Use product_id to filter
...
const clearWishlist = catchAsync(async (req, res, next) => {
  ...
  const { data: removedData, error: removeError } = await supabase
    .from("wishlist_items")
    .delete()
    .eq("wishlist_id", wishlist_id); // Delete items for this user
